...
import { Alert, Box, Button, Card, CardContent, CardHeader, CardTitle, Typography } from "@/core/components/ui";
...
export default function QuizScreen() {
	...
	const question = useMemo(() => {
		return ivcfQuestions.find((item) => item.order === currentQuestion) ?? ivcfQuestions[0];
	}, [currentQuestion]);

	const selectedOptionId = question ? answers[question.id]?.optionId : undefined;
	...
	const primaryButtonDisabled =
		!selectedOptionId || isSubmitting || !participantId || (isLastQuestion && !hasAllAnswers);
				...
				<CardContent className="space-y-6">
					{!participantId && (
						<Alert className="space-y-2 border-amber-300 bg-amber-50 text-amber-800">
							...
							<Typography variant="small">Escolha um participante antes de continuar.</Typography>
							<ParticipantAutocomplete onChange={(value) => setSelectedParticipant(value)} />
						</Alert>
					)}
					...
					{errorMessage && (
						<Alert className="border-destructive bg-destructive/10 text-destructive">
							<Typography variant="small" className="font-medium">{errorMessage}</Typography>
						</Alert>
					)}
					...
					<Box className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
						...
						<Box className="flex gap-3">
							<Button variant="outline" disabled={previousDisabled} onClick={previousQuestion}>
								Anterior
							</Button>
							<Button
								onClick={handleNext}
