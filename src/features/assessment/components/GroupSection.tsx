    ...
    <Box className="flex flex-col gap-2 rounded-md border border-muted px-3 py-2">
      ...
      <Box className="space-y-1">
        {node.answers.map((answer) => (
          <Box key={answer.id} className="flex items-center justify-between text-sm text-muted-foreground bg-muted/20 px-2 py-1.5 rounded">
  ...
  return (
    <Card className="border bg-muted/30 shadow-none">
      <CardHeader className="flex flex-row items-center justify-between p-4 pb-2">
        <Typography variant="small" className="font-semibold uppercase text-muted-foreground">{subgroup.label}</Typography>
    ...
    <details className="rounded-lg border border-border bg-card shadow-sm open:pb-4 group">
      <summary className="flex cursor-pointer items-center justify-between gap-3 px-4 py-4 hover:bg-muted/30 transition-colors">
        ...
        <Box className="flex items-center gap-2">
            <Typography variant="small" className="font-bold text-primary bg-primary/10 px-2 py-1 rounded-md">
            {group.total} pts
            </Typography>
            {group.total !== group.rawTotal && (
                <Typography variant="small" className="text-muted-foreground">
                (bruto {group.rawTotal})
                </Typography>
            )}
